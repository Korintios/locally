...
* **Funcionalidad:**
    * Enviar prompts al backend y recibir respuestas en *streaming*.
...
* **Endpoints (API REST):**
    * `POST /api/chat/stream`: Endpoint principal. Recibe el prompt y el ID de la conversación. Hace *streaming* de la respuesta de Ollama y la guarda en Mongo asincrónicamente.
    * `GET /api/history`: Obtiene la lista de todas las conversaciones.
    * `GET /api/history/{chat_id}`: Obtiene los mensajes de un chat específico.
    * `DELETE /api/history/{chat_id}`: Elimina un chat.
    * `PUT /api/history/{chat_id}`: Renombra un chat.
* **Lógica de Negocio:**
    * Manejar la serialización/deserialización de datos con Pydantic.
    * Conectarse a MongoDB para todas las operaciones de historial.
...
* **Esquemas (Colecciones):**
    * **`conversations`**:
        * `_id`: (ObjectID)
        * `title`: (String, ej. "Chat sobre Python")
        * `created_at`: (Timestamp)
    * **`messages`**:
        * `_id`: (ObjectID)
        * `conversation_id`: (ForeignKey a `conversations`)
